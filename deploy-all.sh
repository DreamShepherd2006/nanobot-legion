...
SOURCE="$SCRIPT_DIR/../repos/nanobot-legion"        # 部署代码源头 (staging 分支)
...
PLATFORM_INFO[hf-staging]="/data/instances/neo/workspace/repos/staging-deploy|git push origin main:main|https://huggingface.co/spaces/DreamShepherd2006/Nanobot-Staging|https://dreamshepherd2006-nanobot-staging.hf.space/health"
PLATFORM_INFO[ms]="/data/instances/neo/workspace/repos/modelscope-deploy|git push origin master|https://www.modelscope.cn/studios/Stone2006/nanobot-multi-agent-nightly|https://stone2006-nanobot-multi-agent-nightly.ms.show/health"
PLATFORM_INFO[nightly]="/data/instances/neo/workspace/repos/nightly-sync|git push origin main|https://huggingface.co/spaces/DreamShepherd2006/nanobot-multi-agent-nightly|https://dreamshepherd2006-nanobot-multi-agent-nightly.hf.space/health"
...
sync_files() {
    ...
    # 脚本目录 (resurrect_neo.sh 等)
    if [ -d "$deploy_dir/scripts" ]; then
        mkdir -p "$dst_dir/scripts"
        cp -r "$deploy_dir/scripts/"* "$dst_dir/scripts/"
    fi
...
deploy_platform() {
    ...
    cd "$dst_dir"
    git config user.name "Nanobot Legion" 2>/dev/null
    git config user.email "legion@nanobot.dev" 2>/dev/null
